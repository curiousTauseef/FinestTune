...
def uniform_embedding(shape, scale=0.05):
    return np.random.uniform(low=-scale, high=scale, size=shape)


def w2v_lookup(alphabet, word2vec_model, dimension, use_binary=True):
    """
    ...
    for w, index in alphabet.iteritems():
        ...
        if w in model:
            ...
            embedding = uniform_embedding([1, dimension])
        # print table[index, :].shape
        # print embedding
